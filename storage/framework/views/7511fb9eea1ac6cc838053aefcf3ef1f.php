...
<?php $component->withAttributes(['title' => 'General Settings','description' => 'Update branding, contact info, and metadata.']); ?>
        <form action="<?php echo e(route('admin.site-settings.update')); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="footer_text" rows="3" class="mt-1 w-full rounded-xl border-slate-200 focus:border-emerald-400 focus:ring-emerald-400"><?php echo e(old('footer_text', $defaultSettings['general']['footer_text'])); ?></textarea>
            </div>
            ...
            <div class="grid gap-6 md:grid-cols-3">
                ...
                ?>
                <?php $__currentLoopData = ['logo' => 'Logo', 'logo_dark' => 'Dark Logo', 'favicon' => 'Favicon']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="h-24 w-24 overflow-hidden rounded-xl border border-slate-200 bg-white">
                                <img src="<?php echo e($placeholders[$key]); ?>" alt="<?php echo e($label); ?>" class="h-full w-full object-cover">
                            </div>
                            <label class="flex w-full cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-2 text-center text-xs font-medium text-slate-500 hover:border-emerald-400 hover:text-emerald-500">
                                ...
                                Upload
                                <input type="file" name="<?php echo e($key); ?>" class="hidden">
                            </label>
                        </div>
                    </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="url" name="facebook_url" value="<?php echo e(old('facebook_url', $defaultSettings['social']['facebook_url'])); ?>" class="mt-1 w-full rounded-xl border-slate-200">
                </div>
                <div>
                    <label class="text-sm font-medium text-slate-700">Twitter URL</label>
                    <input type="url" name="twitter_url" value="<?php echo e(old('twitter_url', $defaultSettings['social']['twitter_url'])); ?>" class="mt-1 w-full rounded-xl border-slate-200">
                </div>
                <div>
                    <label class="text-sm font-medium text-slate-700">Instagram URL</label>
                    <input type="url" name="instagram_url" value="<?php echo e(old('instagram_url', $defaultSettings['social']['instagram_url'])); ?>" class="mt-1 w-full rounded-xl border-slate-200">
                </div>
                <div>
                    <label class="text-sm font-medium text-slate-700">LinkedIn URL</label>
                    <input type="url" name="linkedin_url" value="<?php echo e(old('linkedin_url', $defaultSettings['social']['linkedin_url'])); ?>" class="mt-1 w-full rounded-xl border-slate-200">
                </div>
            </div>
...
<?php echo $__env->make('layouts.admin', ['title' => 'Site Settings'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\wilchar\resources\views/admin/site-settings/edit.blade.php ENDPATH**/ ?>