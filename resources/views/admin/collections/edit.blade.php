...
@section('content_header')
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold">Edit Collection #{{ $collection->id }}</h1>
        <a href="{{ route('collections.show', $collection) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">
    ...
    <form action="{{ route('collections.update', $collection) }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="loan_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach($loans as $loan)
                            <option value="{{ $loan->id }}" {{ old('loan_id', $collection->loan_id) == $loan->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" name="amount" step="0.01" min="0.01" required
                           value="{{ old('amount', $collection->amount) }}"
                ...
                <div>
                    ...
                    <select name="payment_method" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="Mpesa" {{ old('payment_method', $collection->payment_method) == 'Mpesa' ? 'selected' : '' }}>M-Pesa</option>
                        <option value="Bank" {{ old('payment_method', $collection->payment_method) == 'Bank' ? 'selected' : '' }}>Bank Transfer</option>
                ...
                <div>
                    ...
                    <input type="date" name="paid_at" required
                           value="{{ old('paid_at', $collection->paid_at ? \Carbon\Carbon::parse($collection->paid_at)->format('Y-m-d') : now()->format('Y-m-d')) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="text" name="reference" value="{{ old('reference', $collection->reference) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                    ...
                    <input type="url" name="receipt_url" value="{{ old('receipt_url', $collection->receipt_url) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex justify-end gap-4">
                <a href="{{ route('collections.show', $collection) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg">
                    Cancel
                </a>
